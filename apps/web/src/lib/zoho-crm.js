...
async function getCrmAccessToken() {
    ...
    if (!clientId || !clientSecret || !refreshToken) {
        throw new Error('[zoho-crm] Missing credentials: ZOHO_SELF_CLIENT_ID, ZOHO_SELF_CLIENT_SECRET, ZOHO_REFRESH_TOKEN_CORE');
    }
...
// ─────────────────────────────────────────────────────────────────────────────
async function crmFetch(url, options, maxRetries = 5) {
...
export async function upsertCrmContact(params) {
    ...
    if (params.phone)
        contactBody['Phone'] = params.phone;
    if (params.country)
        contactBody['Mailing_Country'] = params.country;
    ...
    const res = await crmFetch(`${ZOHO_CRM_API}/Contacts/upsert?duplicate_check_fields=Email`, {
        method: 'POST',
...
// ─────────────────────────────────────────────────────────────────────────────
export async function updateCrmContactStatus(zohoContactId, status) {
    const token = await getCrmAccessToken();
    const res = await crmFetch(`${ZOHO_CRM_API}/Contacts/${zohoContactId}`, {
        method: 'PUT',
        headers: crmHeaders(token),
        body: JSON.stringify({
 ...
 */
export async function createCrmDeal(params) {
    const token = await getCrmAccessToken();
    const dealBody = {
    ...
    if (params.coach_name)
        dealBody['Description'] = `Coach: ${params.coach_name}`;
    const res = await crmFetch(`${ZOHO_CRM_API}/Deals`, {
        method: 'POST',
        headers: crmHeaders(token),
 ...
 */
export async function checkZohoCustomFields() {
    try {
        const token = await getCrmAccessToken();
        // Use v6 API for broader field coverage; fetch directly (metadata is low-volume)
        const res = await fetch('https://www.zohoapis.com/crm/v6/settings/fields?module=Contacts', {
            method: 'GET',
            headers: crmHeaders(token),
        });
        if (!res.ok) {
            console.warn(`[zoho-crm] Metadata API returned ${res.status} — skipping field check`);
        ...
        if (missing.length > 0) {
            console.warn(`[zoho-crm] MISSING custom fields in Zoho CRM Settings: ${missing.join(', ')}`);
        }
        return { ok: missing.length === 0, missing };
    }
    catch (e) {
        console.warn(`[zoho-crm] Field health check threw error: ${e instanceof Error ? e.message : String(e)} — continuing anyway`);
