...
type SyncOperation = 'upsert_contact' | 'create_deal' | 'update_status';
 ...
 */
export async function enqueueCrmContactSync(params: CrmContactParams & { profile_id: string }): Promise<void> {
...
async function syncOneContact(params: CrmContactParams & { profile_id: string }): Promise<void> {
  ...
  console.log(
    `[crm-sync] Contact ${result.was_existing ? 'updated' : 'created'}: ${params.email} → ${result.zoho_contact_id}`,
  );
}
...
async function getCrmState(profileId: string): Promise<{
  ...
  return withAdminContext(async (db) => {
    const rows = await db.execute(sql`
      ...
      LIMIT 1
    `);
    return (rows.rows[0] as { zoho_contact_id: string | null; activity_status: string } | undefined) ?? null;
  });
}
...
async function markSyncError(profileId: string, error: string): Promise<void> {
  await withAdminContext(async (db) => {
    await db.execute(sql`
...
// ─────────────────────────────────────────────────────────────────────────────
// Batch sync: new / recently updated users (called by 15-min cron)
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export async function runBatchContactSync(limit = 50): Promise<BatchSyncResult> {
...
async function processQueueRow(row: {
  ...
  if (operation === 'upsert_contact') {
    if (!payload.email) throw new Error('upsert_contact payload missing email');
    await syncOneContact({
      profile_id,
...
// ─────────────────────────────────────────────────────────────────────────────
// Daily status refresh (called by daily cron)
// ─────────────────────────────────────────────────────────────────────────────
 ...
 */
export async function runDailyStatusRefresh(limit = 100): Promise<StatusRefreshResult> {
  const result: StatusRefreshResult = { checked: 0, updated: 0, errors: [] };
  ...
  for (const row of contacts) {
    ...
    if (newStatus === row.current_status) continue; // no change — skip API call
