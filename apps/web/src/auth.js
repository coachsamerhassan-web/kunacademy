        ...
        Credentials({
            ...
            async authorize(credentials) {
                ...
                const result = await withAdminContext(async (adminDb) => {
                    const { rows } = await adminDb.execute(sql `SELECT id, email, password_hash, name, image FROM auth_users WHERE email = ${email}`);
                    return rows[0];
                });
        ...
        async jwt({ token, user }) {
            if (user) {
                ...
                const profile = await withAdminContext(async (adminDb) => {
                    const { rows } = await adminDb.execute(sql `SELECT role, preferred_language FROM profiles WHERE id = ${user.id}`);
                    return rows[0];
                });
        ...
        async signIn({ user, account }) {
            ...
            if (account?.provider === 'google') {
                ...
                await withAdminContext(async (adminDb) => {
                    await adminDb.execute(sql `INSERT INTO auth_users (id, email, email_verified, name, image)
                ...
                RETURNING id`);
                    ...
                    const { rows: profileRows } = await adminDb.execute(sql `INSERT INTO profiles (id, email, full_name_en, avatar_url, role)
                ...
                RETURNING xmax::text AS xmax`);
                    isNewProfile = profileRows[0]?.xmax === '0';
                });
