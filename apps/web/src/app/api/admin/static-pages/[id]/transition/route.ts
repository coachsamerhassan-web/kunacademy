/**
 * Wave 15 Wave 3 — Admin transition route for static_pages.
 *
 * Mirrors /api/admin/lp/[id]/transition but for entity='static_pages'.
 * Same R1+R2+R3 lint pipeline; same 422 + content_edits audit shape.
 */
...
export async function POST(request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  if (to === 'review' || to === 'published') {
    ...
    try { row = await loadRowForLint('static_pages', id); }
    catch (err) { console.error('[admin sp transition] preload failed', err); return NextResponse.json({ error: 'preload failed' }, { status: 500 }); }
    if (!row) return NextResponse.json({ error: 'static_pages row not found' }, { status: 404 });

    lintViolations = lintRowBody({ entity: 'static_pages', row });
        ...
        await withAdminContext(async (adminDb) => {
          await adminDb.execute(sql`
            ...
            VALUES ('static_pages', ${id}, '__lint',
                    ...
                    'lint_block',
                    ${`HARD-BLOCK on transition→${to} (admin static_pages): ${audit.rule_ids.join(', ')}`},
                    'admin_ui')
          `);
        });
      } catch (err) { console.error('[admin sp transition] lint_block audit failed', err); }
        ...
        await withAdminContext(async (adminDb) => {
          await adminDb.execute(sql`
            ...
            VALUES ('static_pages', ${id}, '__lint',
                    ...
                    'lint_warn',
                    ${`SOFT-WARN on transition→${to} (admin static_pages)`},
                    'admin_ui')
          `);
        });
      } catch (err) { console.error('[admin sp transition] lint_warn audit failed', err); }
    }
  }

  try {
    const post = await transitionStatus('static_pages', id, to as Status, {
    ...
    return NextResponse.json({ entity: 'static_pages', id, status: post.status, lints: lintViolations.length > 0 ? violationsToResponse(lintViolations) : null });
  } catch (err) {
    console.error('[admin sp transition]', err);
