  ...
  async function fetchData() {
    ...
    if (supabase && user) {
      const { data: earningsData } = await supabase
        ...
        .limit(200);
      setEarnings((earningsData as unknown as Earning[]) ?? []);
    }
    ...
    if (res.ok) {
      ...
      setPayouts((data.payouts as PayoutRequest[]) ?? []);
      setAvailableBalance((data.available_balance as number) ?? 0);
    }
