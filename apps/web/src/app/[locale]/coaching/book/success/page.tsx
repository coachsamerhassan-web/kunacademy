    ...
    <div className="rounded-2xl border border-[var(--color-primary)] bg-white p-5 space-y-4">
      ...
      <div className="relative my-1">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-white px-3 text-[var(--color-neutral-400)]">
            {isAr ? 'أو' : 'or'}
          </span>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={() => {
          // Return to this same page after OAuth so the auto-link effect can fire
          const currentUrl = window.location.href;
          signIn('google', { callbackUrl: currentUrl });
        }}
        className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-[var(--color-neutral-200)] bg-white px-4 py-3 text-sm font-medium text-[var(--color-neutral-700)] hover:bg-[var(--color-neutral-50)] transition-colors min-h-[44px]"
      >
        <GoogleIcon />
      ...
      <div className="text-center pt-1">
        <Link
          ...
          className="text-xs text-[var(--color-neutral-500)] hover:text-[var(--color-primary)] transition-colors"
        >
...
function SuccessPageInner({ locale }: { locale: string }) {
  ...
  // Auto-link: when authenticated user returns here after Google OAuth with a guest booking
  const { user: authUser, loading: authLoading } = useAuth();
  const linkAttempted = useRef(false);

  useEffect(() => {
    // Wait until session is resolved and booking is loaded
    ...
    fetch('/api/bookings/link-account', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      ...
      .then(async (res) => {
        if (res.ok) {
          // Brief delay so user sees the "Linking..." message
          setTimeout(() => {
            window.location.href = `/${locale}/portal/student/bookings`;
          }, 1200);
        } else {
  ...
  if (linkingBooking) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] gap-3" aria-busy="true">
        <div className="w-8 h-8 rounded-full border-2 border-[var(--color-primary)] border-t-transparent animate-spin" />
        <p className="text-sm text-[var(--color-neutral-500)]">
          {isAr ? 'جاري ربط حجزك...' : 'Linking your booking...'}
        </p>
      </div>
    );
  }
