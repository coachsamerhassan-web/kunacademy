/**
 * @kunacademy/email — Wave F.6 membership email templates / shared helpers.
 *
 * Bilingual (AR + EN, both blocks in same body separated by <hr>).
 * Brand-aligned (purple #474099 header, beige #f5f3ef background).
 * IP-clean: NO methodology references — generic membership benefits only.
 *
 * All templates use the same dual-language envelope so the user sees both
 * blocks regardless of stored language preference (matches password-reset
 * pattern from prior waves).
 */

export type Lang = 'ar' | 'en';

export interface MembershipEmailEnvelope {
  /** Email recipient. */
  to: string;
  /** Recipient display name (best-effort fallback to email local-part). */
  recipient_name?: string | null;
  /** Optional Stripe billing portal / dashboard return path. */
  return_url?: string | null;
}

/** Brand palette + styling consistent with password-reset / recording-received. */
export const BRAND = {
  primary: '#474099',
  primaryDark: '#2D2860',
  background: '#f5f3ef',
...
export function formatMoneyMinor(minor: number | null | undefined, currency: string, lang: Lang): string {
 ...
 * The header is rendered in the user-preferred language; the body shows AR
 * block first, then a thin divider, then EN block.
 */
export function buildBilingualEmail(opts: EnvelopeOpts): string {
  const brandSubtitleAr = opts.brandSubtitle?.ar ?? 'أكاديمية كُن للكوتشينج';
  const brandSubtitleEn = opts.brandSubtitle?.en ?? 'Kun Coaching Academy';
          ...
          <tr>
            <td style="background:${BRAND.primaryDark};padding:18px 32px;text-align:center;">
              <p style="margin:0;color:rgba(255,255,255,0.5);font-size:11px;">
                ${brandSubtitleEn} — kunacademy.com
              </p>
            </td>
          </tr>
        </table>
...
export function buildLangBlock(args: {
  ...
  const toneStyle = (() => {
    switch (tone) {
      ...
      case 'success':
        return `background:${BRAND.accentGreen};border:1px solid ${BRAND.accentGreenBorder};color:${BRAND.accentBlueText};`;
      default:
